...
def train(nb_itt, train_loss, test_loss, resample_rate, display, poids,
    ...
    for epoch in range(len(train_loss), nb_it_tot) : 
        ...
        # loss totale
        loss_test = poids[0]*loss_test_data + poids[1]*loss_test_pde # + poids[1]*loss_test_bc + poids[2]*loss_test_ic
        ...
        if ((epoch+1) % display == 0) or (epoch+1 == nb_it_tot):
            ...
            #print(f"Train : loss: {np.mean(train_loss[-display:]):.3e}, pde: {loss_pde:.3e}, bc: {loss_bc:.3e}, ic: {loss_ic:.3e}")
            #print(f"Test  : loss: {np.mean(test_loss[-display:]):.3e}, pde: {loss_test_pde:.3e}, bc: {loss_test_bc:.3e}, ic: {loss_test_ic:.3e}")
